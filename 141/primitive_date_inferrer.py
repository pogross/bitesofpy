...
def get_dates(dates):
    ...
    formats = []
    for date in dates:
        formats.extend(_maybe_DateFormats(date))
    top_formats = Counter(formats).most_common(2)

    if top_formats[0][1] == top_formats[1][1]:
        raise InfDateFmtError
    if top_formats[0][0] == DateFormat.NONPARSABLE:
        raise InfDateFmtError

    inferred_dates = []
    for date in dates:
        try:
            inf_date = datetime.strptime(
                date, DateFormat.get_d_parse_formats(top_formats[0][0].value)
            )
            inferred_dates.append(datetime.strftime(inf_date, "%Y-%m-%d"))
        except ValueError:
            inferred_dates.append("Invalid")

...
def make_character_index(
    ...
    for nr, line in enumerate(lines):
        for character in characters:
            # check if synonymes exist
            if type(character) == tuple:
                for synonym in character:
                    if synonym.lower() in line.lower():
                        character_index[character[0].lower()].append(nr)
                        break  # avoid counting synonym multiple times!
            else:
                if character.lower() in line.lower():
                    character_index[character.lower()].append(nr)

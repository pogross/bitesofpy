...
def get_movies():
    ...
    return [movie.get("title") for movie in get_tree().getroot().findall("movie")]
...
def get_movie_longest_runtime():
    ...
    runtimes = [(movie.get("title"), movie.get("runtime")) for movie in get_tree().getroot().findall("movie")]

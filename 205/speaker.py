...
def get_pycon_speaker_first_names(soup: Soup = _get_soup()) -> List[str]:
    ...
    """
    raw_speakers = [entry.text.strip() for entry in soup.findAll("span", {"class": "speaker"})]
    speakers = [
        speaker.strip() for speakers in raw_speakers for speaker in re.split(r"[\,/]", speakers)
    ]
...
def get_percentage_of_female_speakers(first_names: list) -> float:
    ...
    female_percantage = (count["female"] + count["mostly_female"]) / len(first_names) * 100

...
def _is_alive(person: Composer, date: str) -> bool:
    _dt = _get_date(date)
    if _get_date(person.born) < _dt and _get_date(person.died) > _dt:
        return True

    return False


def operas_both_at_premiere(guest: str, composer: str) -> List[Opera]:
    ...
    titles = [
        ...
        and _is_alive(composers[guest], opera.date)
        and _is_alive(composers[composer], opera.date)
    ]
